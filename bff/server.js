...
Issuer.discover(pingIssuerUrl)
  .then(issuer => {
    ...
    app.get('/initiate-ping-login', (req, res, next) => {
      ...
      if (correlationId) {
        req.session.correlationId = correlationId; // Store correlationId in session
    ...
    app.get('/auth/callback', async (req, res, next) => {
      ...
      const correlationId = req.session.correlationId; // Retrieve correlationId from session
      ...
      try {
        ...
        if (!req.session.oidcState || params.state !== req.session.oidcState) {
          ...
          if (req.session.correlationId) delete req.session.correlationId; // Clean up session
          return next(new Error('State mismatch or session state missing.'));
        }
        delete req.session.oidcState;
        // Keep req.session.correlationId for the /exchange-code step if needed there, or move it.
        // For now, it's logged. It will be cleared if /exchange-code uses it.

        req.session.oidcCallbackParams = params; // Contains 'code' and 'state'
    ...
    app.get('/exchange-code', async (req, res, next) => {
      ...
      const correlationId = req.session.correlationId; // Retrieve correlationId
      ...
      // We can also delete correlationId from session now as its journey leg for login is complete.
    ...
    app.post('/api/introspect-token', async (req, res, next) => {
      const correlationId = req.headers['x-correlation-id']; // Assuming client sends it
      console.log(`/api/introspect-token hit. Correlation ID from header: ${correlationId || 'N/A'}`);
    ...
    app.get('/api/user', (req, res) => {
      const correlationId = req.headers['x-correlation-id']; // Assuming client sends it
      console.log(`/api/user hit. Correlation ID from header: ${correlationId || 'N/A'}`);
    ...
    app.get('/logout', async (req, res, next) => {
      const correlationId = req.session.correlationId || req.query.correlationId; // May or may not be in session
      ...
      req.session.destroy(err => {
        ...
        try {
          const endSessionUrl = oidcClient.endSessionUrl({
            ...
            post_logout_redirect_uri: frontendRedirectUrl,
          });
    ...
    app.use((err, req, res, next) => {
      const correlationId = req.session && req.session.correlationId ? req.session.correlationId :
                           (req.query && req.query.correlationId ? req.query.correlationId :
                           (req.headers && req.headers['x-correlation-id'] ? req.headers['x-correlation-id'] : 'N/A'));
